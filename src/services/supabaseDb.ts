  ...
  async getAttendanceRecords(orgId?: string): Promise<AttendanceRecord[] | null> {
    ...
    try {
      ...
      return data.map(r => ({
        ...
        status: (r.status?.toUpperCase() === 'LATE' || r.is_late) ? 'LATE' : (r.status?.toUpperCase() === 'ON_BREAK' ? 'ON_BREAK' : (r.status?.toUpperCase() === 'PRESENT' || r.status === 'Completed' || r.status === 'Working') ? 'PRESENT' : 'PRESENT'),
        completionStatus: r.completion_status || 'Working',
